...
namespace MySoft.Project.Excel
{
    ...
    public class CellTemplateToken : ITemplateToken
    {
        public int RowIndex { get; set; }
        public int ColIndex { get; set; }
        public string BindName { get; set; }       
    }
    public class TableTemplateToken : CellTemplateToken
    {
     
        public string ValueName { get; set; }
        public int EndRowIndex { get; set; }
        public int EndColIndex { get; set; }
        public List< CellTemplateToken> Cells { get; set; }
        public TableTemplateToken() {
            Cells = new List<CellTemplateToken>();
        }
    ...
    {
        public List<CellTemplateToken> Cells { get; set; }
        public List<TableTemplateToken> Tables { get; set; }
    }
    public class ExcelTemplateParser
    {
        ...
        TableTemplateToken ParseTable(ToxyCell cell)
        {
            ...
            TableTemplateToken token = new TableTemplateToken() { RowIndex = _currRowIndex, ColIndex = _currColIndex, BindName = eachpart[0].Value };
            ...
            if (eachpart.Count == 1)
            {
                ...
                if (valGroup.Count == 0)
                {
                    token.Cells.Add(new CellTemplateToken() { RowIndex = _currRowIndex, ColIndex = _currColIndex });
                    _currColIndex++;
                }
                else
                {
        ...
        CellTemplateToken ParseCell(ToxyCell cell)
        {
            if (cell == null || string.IsNullOrEmpty(cell.Value)) {
                return null;
            }
            ...
            if (valGroup.Count == 1)
            {
                return new CellTemplateToken() { RowIndex = _currRowIndex, ColIndex = _currColIndex, BindName = valGroup[0].Value };
            }
        ...
        public TableTemplateToken Parse(string filepath)
        {
